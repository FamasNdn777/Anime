...
app.get('/api/pesquisar', async (req, res) => {
    ...
    async function collectUnique(urls) {
        ...
        settled.forEach(result => {
            ...
            result.value.forEach(anime => {
                ...
                seen.add(anime.slug); unique.push(anime);
            });
        });
        return unique;
    }
    function filterMatches(items) {
        ...
        return items.filter(a => normalizeText(a.titulo).includes(nq));
    }
    function buildPrimaryUrls() {
        ...
        return [
            ...
            'https://animefire.io/em-lancamento/1', 'https://animefire.io/em-lancamento/2'
        ];
    }
...
function extractImg($el) {
    ...
    return img.attr('data-src') || img.attr('data-lazy-src') || img.attr('data-original')
        || (img.attr('srcset') || '').split(',')[0].trim().split(' ')[0]
        || img.attr('src') || '';
}
function extractTitle($el) {
    return $el.find('.data h3 a').text().trim() || $el.find('.data h3').text().trim()
        || $el.find('h3 a').text().trim() || $el.find('h3').text().trim()
...
async function scrapeAnimesOnline(url, res) {
    try {
        ...
        const $ = cheerio.load(data);
        const animes = []; const seen = new Set();
        $('article.item, article.item.tvshows, .result-item, .search-page .result-item, div.TPost').each((i, el) => {
            ...
            if (slug && titulo && !seen.has(slug)) { seen.add(slug); animes.push({ titulo, slug, capa }); }
        });
        res.json(animes);
    } catch (e) {
...
app.get('/api/s2/lancamentos', async (req, res) => {
    const page = req.query.page || 1;
    ...
    try {
        ...
        if (animes.length > 0) return res.json(animes);
...
app.get('/api/s2/detalhes', async (req, res) => {
    ...
    try {
        ...
        $('.episodios li, #episodios li, .episodelist li').each((i, el) => {
            ...
            const nome = linkEl.text().trim() || $(el).find('.numerando').text().trim() || 'Episódio ' + (i + 1);
...
if (require.main === module) {
    app.listen(3000, '0.0.0.0', () => {
        console.log('🐉 DRAKSYON SERVER — ONLINE na porta 3000');
    });
}
